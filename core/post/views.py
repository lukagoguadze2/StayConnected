...
class PostView(mixins.ListModelMixin,
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        instance = get_object_or_404(
            Post.objects.annotate_with_seen_by_user(user=self.request.user), pk=self.kwargs['pk']
        )
        instance.seen_by_user = True
        serializer = self.get_serializer(instance, context={'request': request})
        return Response(serializer.data)

    @action(detail=False, methods=['post'], serializer_class=CreatePostSerializer, name='create_post', url_path='create')
